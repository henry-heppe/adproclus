...
plot_cluster_network <- function(model,
        ...
        checkmate::assertChoice(filetype, c("R", "pdf", "svg", "tex", "jpg", "tiff", "png", ""), null.ok = TRUE)
...
#'
#' Produce a representation of profile matrix \eqn{\boldsymbol{P}} of a (low dimensional) ADPROCLUS solution
#' of class \code{adpc}. The plot displays the profiles in the style of a correlation plot.
#' \strong{NOTE:} This function can also be called through the \code{plot(model, type = "Profiles")} function
...
plot_profiles <- function(model, title = "Profiles of ADPROCLUS solution") {
        ...
        if(is.null(model$C)) {
                corrplot::corrplot(model$P, is.corr = FALSE, title = title, mar = c(0,0,2,0))
        } else {
                if (title == "Profiles of ADPROCLUS solution") {
                        title = "Profiles of Low dimensional ADPROCLUS solution"
                }
                corrplot::corrplot(model$C, is.corr = FALSE, title = title, mar = c(0,0,2,0))
        }
        invisible(model)
...
#'
#' Produce a representation of variable to component matrix \eqn{\boldsymbol{B'}} of a \strong{low dimensional} ADPROCLUS solution
#' of class \code{adpc}. The plot displays the scores in the style of a correlation plot.
#' \strong{NOTE:} This function can be called through the \code{plot(model, type = "VarsByComp")} function
#' with model an object of class \code{adpc}.
#'
